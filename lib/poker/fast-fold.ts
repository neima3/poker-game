...
import type { GameState, GameMode, BotDifficulty } from '@/types/poker';
 ...
 * The player always gets a random seat position for variety.
 */
export function dealFastFoldHand(session: FastFoldSession): GameState {
  const tableId = `ff_table_${session.sessionId}`;

  // Random seat for the player (1-6)
  const humanSeat = Math.floor(Math.random() * session.tableSize) + 1;
  ...
  for (let seat = 1; seat <= session.tableSize; seat++) {
    if (seat === humanSeat) {
      ...
      // Vary bot stacks slightly for realism
      const stackVariance = 0.7 + Math.random() * 0.6; // 70-130% of player stack
  ...
  // Random dealer position for variety
  const dealerSeat = Math.floor(Math.random() * session.tableSize) + 1;

  let gameState = dealHoleCards(
    initGame(tableId, players, session.smallBlind, session.bigBlind, dealerSeat, 'fast_fold')
  );

  // Process leading bot turns
